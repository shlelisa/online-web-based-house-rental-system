...
if(empty(trim($user)) ||empty(trim($ema)) ||empty(trim($pass)) ||empty(trim($rol))||empty(trim($address))||empty(trim($phone)) ){
...
$updateq="UPDATE new_user SET username='$user',email='$ema',password='$pass',role='$rol',photo='$phot',address='$address',phone='$phone' WHERE id='$id'";
	$run_updateq=mysqli_query($conn,$updateq);
	if($run_updateq)
	{
		$message="UPDATED";
		#header("location:userAccount.php");
		echo'<script type="text/javascript">window.location="userAccount.php"; </script>';
	}
	else
	{
...
input{
 ...
 <table border="1" style="background-color:whitesmoke">
     ...
     <td width="311">
      <form style="text-align:center" method="post" enctype="multipart/form-data" action="updateuserdata.php">
     <br><br>
         ...
         <br>
          ...
          <label for="id">Password: </label><br />
           ...
           <label for="fname2">Address: </label><br />
            <input style="width:300px;height:30px" type="text" name="address" id="address" value="<?php echo $address?>" /><br />
         <br>
		   <label for="fname2">Phone: </label><br />
            <input style="width:300px;height:30px" type="number" name="phone" id="phone" value="<?php echo $phone?>" /><br />
         <br>
